...
class CryptoDataSource(BaseDataSource):
    ...
    def _get_kline_ktx(
        ...
        When the primary market_type returns no candles, automatically retry the
        alternate market type (spot <-> swap) as a fallback.
        """
        try:
            from app.services.live_trading.ktx import KtxClient
            ...
            # Build the list of market types to try: primary first, then the fallback.
            ...
            if not klines:
                logger.warning(
                    f"KTX get_kline returned no candles for {symbol} {timeframe} "
                    f"(tried market_types={market_types_to_try})"
                )
                return []
            ...
            if klines:
                try:
                    ...
                    logger.info(
                        f"[CryptoKline] {symbol} {timeframe} returned {len(klines)} candles (KTX native, market_type={used_mt}), "
